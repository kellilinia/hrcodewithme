...
def is_employer(account_data: dict = Depends(authenticator.get_current_account_data)):
    coder = account_data.get("coder")
    if coder is False:
        return True
    else:
        raise HTTPException(status_code=405, detail="You do not have an employer account to access this page.")


@router.get("/employer/search", response_model=List[SearchOut])
def employer_search(search_criteria: SearchIn = Depends(),
